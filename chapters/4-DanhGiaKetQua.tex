...
\begin{table}[!htbp]
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} ll ccccc cc c}
        \toprule
        \multirow{2.5}{*}{\textbf{Pipe.}} & \multirow{2.5}{*}{\textbf{Metrics}} & \multicolumn{2}{c}{\textbf{prepare}} & \multirow{2}{*}{\makecell{\textbf{merge}\\\textbf{data}}} & \multirow{2}{*}{\makecell{\textbf{sec.}\\\textbf{check}}} & \multirow{2}{*}{\makecell{\textbf{pre-}\\\textbf{process}}} & \multicolumn{2}{c}{\textbf{train}} & \multirow{2}{*}{\makecell{\textbf{com-}\\\textbf{pare}}} \\
        ...
        & Duration (s) & --- & 16 & --- & 103 & 111 & 101 & 82 & 27 \\
        & Avg. CPU (c) & --- & 0,05 & --- & 0,26 & 0,19 & 0,23 & 0,05 & 0,05 \\
        & Peak CPU (c) & --- & 0,07 & --- & 0,31 & 0,25 & 0,27 & 0,06 & 0,05 \\
        & Avg. Mem (MB) & --- & 20 & --- & 1.350 & 500 & 750 & 400 & 40 \\
        & Peak Mem (MB) & --- & 30 & --- & 1.600 & 600 & 850 & 430 & 50 \\
        
        \midrule
        
        % --- RETRAINING ---
        \multirow{5}{*}{\rotatebox[origin=c]{90}{\textbf{Retraining}}} 
        & Duration (s) & 37 & 16 & 83 & 138 & 129 & 110 & 89 & 27 \\
        & Avg. CPU (c) & 0,11 & 0,05 & 0,27 & 0,29 & 0,31 & 0,25 & 0,06 & 0,05 \\
        & Peak CPU (c) & 0,15 & 0,07 & 0,35 & 0,35 & 0,38 & 0,3 & 0,06 & 0,05 \\
        & Avg. Mem (MB) & 80 & 20 & 750 & 1.450 & 580 & 820 & 420 & 40 \\
        & Peak Mem (MB) & 100 & 30 & 900 & 1.700 & 680 & 950 & 440 & 50 \\
        \bottomrule
    \end{tabular*}
\end{table}


Hai chỉ số hiệu năng trong lúc quy trình MLSecOps thực thi mà nhóm theo dõi là CPU và memory. Các chỉ số này được Prometheus thu thập trong quá trình thực thi quy trình huấn luyện mô hình, và được trực quan hóa bằng Grafana. Cụ thể hai chỉ số được theo dõi là rate container cpu usage seconds - cho biết mức nhân CPU mà pod đang sử dụng trung bình, và container memory working set bytes - đo lường bộ nhớ 
được pod tiêu thụ tại một thời điểm.

Mục tiêu của nhóm khi lựa chọn hai chỉ số này là để có cái nhìn định lượng về mỗi bước trong quy trình MLSecOps đang tính toán bao nhiêu và giữ bao nhiêu dữ liệu, qua đó có thể xác định được bottleneck và tối ưu tài nguyên.

\subsubsection{Mức nhân CPU pod sử dụng trung bình}
...
Hình \ref{fig:chap4-grafana-cpu} thể hiện mức nhân CPU mà pod trong hệ thống MLSecOps sử dụng trung bình theo thời gian trong quá trình huấn luyện mô hình. Trong biểu đồ này, trục hoành biểu diễn mốc thời gian, trong khi tục tung biểu diễn mức sử dụng CPU của từng container được đo theo đơn vị nhân CPU. Các đường trong biểu đồ tương ứng với một pod trong từng bước của quy trình MLSecOps.

Bảng \ref{tab:cpu_usage_per_pod} trình bày mức sử dụng CPU của từng pod tương ứng với các bước trong pipeline MLsecOps. Kết quả cho thấy bước security check và train brf là hai bước tiêu thụ nhiều tài nguyên CPU nhất. Trong đó, bước security check tiêu thụ CPU cao nhất (0.31 nhân), cho ta thấy đây là bước có cường độ tính toán cao và có khả năng trở thành nút thắt hiệu năng của pipeline. Các bước khác như get base data và train-xgb có mức sử dụng CPU thấp và thời gian thực thi ngắn, cho thấy các bước này không tạo áp lực đáng kể lên tài nguyên hệ thống.
...
\subsubsection{Bộ nhớ pod tiêu thụ}

\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.6]{img/chapter4/grafana-memory-time-series.png}
    \caption{Biểu đồ bộ nhớ pod tiêu thụ tại từng thời điểm}
    \label{fig:chap4-grafana-memory}
\end{figure}

Hình \ref{fig:chap4-grafana-memory} thể hiện mức sử dụng bộ nhớ của các pod trong quy trình MLSecOps theo thời gian. Trong biểu đồ này, trục hoành biểu diễn mốc thời gian, trong khi trục tung biểu diễn lượng bộ nhớ mà mỗi pod sử dụng, tính theo đơn vị megabyte. Các đường trong biểu đồ tương ứng với một pod trong từng bước của quy trình huấn luyện dữ liệu của hệ thống MLSecOps.

Bảng \ref{tab:memory_usage_per_pod} trình bày mức sử dụng bộ nhớ của từng pod tương ứng với các bước trong pipeline MLsecOps. Trong đó, bước security check tiêu thụ lượng bộ nhớ lớn nhất, với giá trị cao nhất khoảng 1.6 GB và duy trì ổn định trong suốt quá trình thực thi. Bước train brf cũng sử dụng lượng bộ nhớ đáng kể do phải lưu trữ dữ liệu huấn luyện và tham số mô hình. Các bước còn lại như preprocess data, train-xgb và get base data có mức sử dụng bộ nhớ thấp hơn và không tạo áp lực đáng kể lên tài nguyên hệ thống.
...
Phân tích trên về mức sử dụng CPU và bộ nhớ của các bước trong pipeline MLsecOps cho thấy tài nguyên hệ thống được sử dụng không đồng đều giữa các bước. Một số bước chỉ tiêu thụ lượng tài nguyên nhỏ, trong khi một số bước khác có yêu cầu tính toán và lưu trữ dữ liệu lớn hơn đáng kể. Nhìn chung, các phân tích này cung cấp cơ sở cho việc điều chỉnh cấu hình tài nguyên và cải thiện khả năng mở rộng của hệ thống MLsecOps.
