...
\begin{itemize}
    ...
    \item Quy trình huấn luyện mô hình: Thực hiện việc xây dựng mô hình phát hiện gian lận dựa trên dữ liệu huấn luyện ban đầu. Quy trình này bao gồm các bước xử lý dữ liệu, huấn luyện và đánh giá mô hình nhằm lựa chọn mô hình phù hợp.
...
Trong phạm vi đề tài này, chúng tôi sử dụng bộ dữ liệu Bank Account Fraud Dataset Suite (NeurIPS 2022). Tệp cơ sở (Base) của bộ dữ liệu bao gồm 1 triệu mẫu và 32 đặc trưng (được mô tả trong Bảng \ref{tab:dataset_feature_describe}), mỗi mẫu đại diện cho một đơn đăng ký mở tài khoản ngân hàng được tạo bằng CTGAN (Conditional Tabular Generative Adversarial Network). Mô hình sinh dữ liệu CTGAN được huấn luyện trên một bộ dữ liệu thật đã được ẩn danh, vì vậy các mẫu được tạo ra không gắn với bất kỳ cá nhân nào trong thực tế, nhưng vẫn giữ được đặc trưng thống kê của dữ liệu thật.
...
Quá trình tiền xử lý dữ liệu đóng vai trò then chốt trong việc xây dựng mô hình học máy hiệu quả. Dựa trên những quan sát từ quá trình Phân tích khám phá dữ liệu (\ac{eda}) và đặc thù của bộ dữ liệu Bank Account Fraud, chúng tôi tiến hành xử lý dữ liệu trước khi đưa vào huấn luyện như sau:
\begin{itemize}
...
Biến mục tiêu fraud\_bool được tách riêng làm nhãn \texttt{y}, các biến còn lại được sử dụng làm tập đặc trưng đầu vào \texttt{X}. Các thuộc tính dạng phân loại cũng được chuyển về kiểu category.
...
Để giảm thiểu rủi ro và tác động xấu của loại tấn công này vào dữ liệu huấn luyện, trong phần này, nhóm sinh viên đề xuất một phương pháp phát hiện tấn công lật nhãn dựa trên dự đoán của mô hình học máy. Ý tưởng chính của phương pháp này là khai thác sự không tương đồng giữa nhãn quan sát được trong dữ liệu và xác suất dự đoán mà mô hình đưa ra. Bên cạnh đó, phương pháp còn dựa vào sự khác biệt trong kết quả dự đoán giữa các mô hình học máy khác nhau, cũng như mức độ tự tin của mô hình khi đưa ra dự đoán đối với các điểm dữ liệu nằm gần ranh giới giữa hai lớp.
...
$r \in (0,1)$ là tham số xác định tỷ lệ dữ liệu nghi ngờ được lựa chọn sau bước xếp hạng. Cụ thể, sau khi các điểm dữ liệu được sắp xếp theo mức độ nghi ngờ lật nhãn, thuật toán chỉ giữ lại $K = \lceil r \cdot N \rceil$ điểm dữ liệu có điểm số cao nhất để đưa vào tập nghi ngờ, phù hợp với giới hạn kiểm tra thủ công trong quy trình human-in-the-loop.
...
\begin{itemize}
    \item \texttt{provider.tf}: Khai báo các provider và cấu hình backend từ xa trên Terraform Cloud để lưu trữ và quản lý trạng thái hạ tầng.
    
    \item \texttt{variables.tf}: Khai báo toàn bộ các biến cấu hình đầu vào cho hệ thống, bao gồm thông tin vùng triển khai, cấu hình mạng (CIDR, \ac{az}), cấu hình cụm EKS, node group, các addon của Kubernetes và các tham số liên quan đến dịch vụ RDS.
    
    \item \texttt{locals.tf}: Định nghĩa các biến cục bộ phục vụ cho quá trình triển khai.
    
    \item \texttt{main.tf}: Là tệp đóng vai trò chính, khai báo các tài nguyên hạ tầng của hệ thống.
    
    \item \texttt{eks\_iam\_role.tf}: Khai báo các \ac{iam} Role và Policy cần thiết cho EKS Control Plane, EKS Node Group và các thành phần liên quan.
    
    \item \texttt{elb\_controller.tf}: Khai báo IAM Role, Policy và cấu hình Pod Identity cho AWS Load Balancer Controller, đồng thời triển khai các thành phần ingress-nginx và cert-manager thông qua Helm.
    
    \item \texttt{cluster\_autoscaler.tf}: Triển khai metrics-server và cluster-autoscaler bằng Helm nhằm tự động điều chỉnh số lượng node trong cụm EKS dựa trên tải hệ thống.
    
    \item \texttt{AWSLoadBalancerControllerIAMPolicy.json}: Chứa định nghĩa policy IAM phục vụ cho AWS Load Balancer Controller và các thành phần liên quan.
\end{itemize}
...
Nhằm phục vụ cho các bước trong các luồng pipeline ở các giai đoạn tiếp theo, chúng tôi xây dựng một tập hợp các Docker image cho mã nguồn của hệ thống. Toàn bộ các image này được xây dựng từ Python, với đầy đủ thư viện và các dependency cần thiết đã được cấu hình sẵn. Chức năng cụ thể của từng image được mô tả như sau:
...
\textbf{Đầu ra:}
\begin{itemize}
    \item Model version được đăng ký trong MLflow Model Registry với tên fraud-detection-model
    \item Báo cáo so sánh chi tiết về metrics và aggregate score của hai mô hình
\end{itemize}
...
\textbf{Đầu vào:}
\begin{itemize}
    \item MODEL\_VERSION: Số version của model cần triển khai từ MLflow Registry
    \item MODEL\_NAME: Tên model trong registry ("fraud-detection-model")
    \item model-serving.yaml: Kubernetes manifest template cho deployment
\end{itemize}

\textbf{Quy trình xử lý:}
\begin{itemize}
    \item Xác thực model version tồn tại trong MLflow Registry và kiểm tra trạng thái READY
    \item Đọc Kubernetes manifest template và thay thế các placeholders (MODEL\_VERSION, DEPLOYMENT\_NAME, CONTAINER\_IMAGE) bằng giá trị thực tế
    \item Áp dụng manifest lên cụm \ac{eks} sử dụng kubectl apply và theo dõi trạng thái rollout deployment với timeout 300 giây
    \item Thực hiện health check endpoint để đảm bảo model server hoạt động bình thường, tự động cleanup nếu deployment thất bại
\end{itemize}

\textbf{Đầu ra:}
\begin{itemize}
    \item Kubernetes deployment và service đang chạy trong namespace mlflow
    \item Model serving endpoint sẵn sàng nhận requests tại endpoint \texttt{/invocations} thông qua \ac{dns} nội bộ của pod \texttt{fraud-detection-model-svc.mlflow.svc.cluster.local}.

\end{itemize}

\subsubsection{mlflow\_serving}
Mục tiêu: Cung cấp image chứa MLflow model serving runtime để triển khai và phục vụ các mô hình trong production.

\textbf{Đầu vào:}
\begin{itemize}
    \item Base image Python slim
    \item MLflow và các dependencies (boto3, imbalanced-learn, xgboost)
\end{itemize}

\textbf{Quy trình xử lý:}
\begin{itemize}
    \item Cài đặt MLflow và các thư viện cần thiết để load và serve model
    \item Thiết lập ENTRYPOINT là lệnh mlflow để container có thể chạy MLflow commands trực tiếp
    \item Khi deploy, container sẽ chạy lệnh mlflow models serve với model URI từ Model Registry và expose HTTP endpoint trên port 8080
    \item Cung cấp health check endpoint tại /health và prediction endpoint tại /invocations để nhận requests dự đoán
\end{itemize}
...
\begin{itemize}
    \item Điều kiện kích hoạt: Được kích hoạt thủ công thông qua cơ chế \textit{workflow\_dispatch} của GitHub Actions. Người dùng lựa chọn thành phần MLSecOps cần xây dựng (ví dụ: \texttt{preprocess\_data}, \texttt{train\_brf}, \texttt{train\_xgb}), phiên bản dữ liệu tương ứng thông qua tệp \texttt{data.dvc}, và chỉ định việc có cập nhật dữ liệu mới hay không.
    \item Mô tả các bước:
    \begin{itemize}
        ...
        \item Quản lý phiên bản dữ liệu (Data versioning): Nếu dữ liệu có sự thay đổi và tham số \texttt{dvc\_push} được bật, thì workflow sẽ thực hiện ghi nhận dữ liệu mới thông qua \ac{dvc}.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.38]{img/chapter3/chap3-ci-build-image.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.38]{img/chapter3/chap3-ci-create-release.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.38]{img/chapter3/chap3-ci-trivy.png}
...
\begin{itemize}
    ...
    \item Mô tả các bước:
    \begin{itemize}
        \item Thiết lập môi trường (Set up environment): Workflow khởi tạo môi trường thực thi bằng cách cài đặt các công cụ cần thiết để tương tác với cụm EKS, chẳng hạn như \texttt{kubectl}, \texttt{Helm} và Argo CD CLI.
        ...
        Workflow áp dụng tệp \texttt{bootstrap.yaml} để tạo các Argo CD Application. Sau đó, Argo CD bắt đầu theo dõi repository GitHub và tự động đồng bộ các Helm chart và manifest Kubernetes tương ứng với trạng thái mong muốn của hệ thống.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.33]{img/chapter3/chap3-ci-gh-chart.png}
...
\paragraph{a. Terraform Plan Workflow}: Đây là workflow được sử dụng để kiểm tra các thay đổi về Terraform khi có Pull Request gộp vào nhánh \texttt{master}. Mục tiêu của workflow là phát hiện sớm các thay đổi không mong muốn, đánh giá tác động của thay đổo và kiểm tra các rủi ro bảo mật trước khi triển khai lên AWS.

Chi tiết về workflow:
\begin{itemize}
    \item Điều kiện kích hoạt: Được kích hoạt tự động khi có Pull Request gộp vào nhánh \texttt{master}. 
    \item Mô tả các bước:
    \begin{itemize}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.33]{img/chapter3/chap3-ci-terraform-plan.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.33]{img/chapter3/chap3-ci-checkov.png}
    \caption{Kết quả quét bảo mật sử dụng Checkov}
    \label{fig:chap3-ci-checkov.png}
\end{figure}

\paragraph{b. Terraform Apply Workflow} Đây là workflow đảm nhiệm vai trò áp dụng các thay đổi hạ tầng đã được phê duyệt lên môi trường AWS. Workflow này chỉ được kích hoạt khi Pull Request vào nhánh \texttt{master} đã được hợp nhất thành công.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Áp dụng thay đổi hạ tầng (Terraform Apply): Lệnh \texttt{terraform apply} được thực thi với tuỳ chọn tự động phê duyệt, từ đó triển khai các dịch vụ lên nền tảng AWS. Các dịch vụ này đã được mô tả trong phần \ref{section-deploy-iac}.
    \end{itemize}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.33]{img/chapter3/chap3-ci-terraform-apply.png}
...
\begin{enumerate}
    ...
    \item Serving Deployment Pipeline - Triển khai mô hình lên môi trường serving
\end{enumerate}

\subsubsection{Template-based Architecture}

Để hiện thực các pipeline trên Argo Workflows, chúng tôi sử dụng kiến trúc dựa trên template (Template-based Architecture). Trong kiến trúc này, mỗi bước trong \ac{dag} được định nghĩa bởi một template độc lập, giúp tối ưu việc quản lý, tái sử dụng và mở rộng cho từng bước đơn lẻ trong pipeline.

Nhằm phục vụ cho cả ba pipeline DAG đã nêu, chúng tôi xây dựng sẵn các template sau, trong đó mỗi template sử dụng container image tương ứng đã được container hóa và lưu trữ trên \ac{ecr}:

\begin{itemize}
    \item \textbf{tp-get-base-data.yaml}: Sử dụng image amazon/aws-cli để trích xuất dữ liệu huấn luyện gốc (base training data) từ S3 production bucket về workspace artifact bucket. Template này thực thi các lệnh AWS CLI để tải xuống file Base.csv từ S3 path được chỉ định và tải lên artifact bucket dùng chung của workflow.
    
    \item \textbf{tp-fetch-prod-data.yaml}: Sử dụng image fetch\_prod\_data chứa Python script và dependencies cần thiết để kết nối DynamoDB, truy xuất dữ liệu production mới theo tham số max-items, và xuất ra file CSV. Container chạy script fetch\_prod\_data.py với các biến môi trường được cấu hình từ workflow parameters.
    ...
    \item \textbf{tp-preprocess-data.yaml}: Sử dụng image preprocess\_data để thực hiện tiền xử lý. Container tải download dữ liệu thô, chạy script preprocess\_data.py để làm sạch, mã hóa và chuẩn hóa dữ liệu, sau đó tải encoders, scalers và dữ liệu đã xử lý lên S3.
    
    \item \textbf{tp-train-xgb.yaml}: Sử dụng image train\_xgb chứa XGBoost library và MLflow client. Container tải dữ liệu preprocessed, thực thi train\_xgb.py để huấn luyện model với kết nối MLflow tracking URI, log metrics/parameters, và upload model artifacts về S3.
    
    \item \textbf{tp-train-brf.yaml}: Sử dụng image train\_brf tương tự train\_xgb nhưng chứa thư viện imblearn. Container thực thi train\_brf.py để huấn luyện Balanced Random Forest model, kết nối MLflow tracking URI, log metrics/parameters, và upload model artifacts về S3.
    ...
    \item \textbf{tp-deploy-model.yaml}: Sử dụng image deploy\_model chứa kubectl CLI và Python SDK. Container chạy deploy\_model.py để generate Kubernetes manifest với model version cụ thể, apply manifest lên cluster, và triển khai model server sử dụng image mlflow\_serving đã được xây dựng trước đó.
\end{itemize}
...
Quy trình Training Pipeline được chúng tôi đề xuất được mô tả trong Hình \ref{fig:chap3-training-pipeline.png}. Trong đó, các bước của \ac{dag} lần lượt được ánh xạ tới những template tương ứng đã trình bày ở mục trước. Với các bước cần thực thi trong container, image sẽ được lấy về từ \ac{ecr} thông qua các tham số cấu hình (ecr-registry, image-tag). Các artifact sau mỗi bước được lưu trữ vào S3 artifact bucket dùng chung, được tổ chức theo ID của workflow, nhằm phục vụ cho các bước tiếp theo và đảm bảo khả năng truy vết, tái lập toàn bộ quá trình huấn luyện.
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{0.1\textwidth} >{\raggedright\arraybackslash}p{0.35\textwidth} >{\raggedright\arraybackslash}p{0.45\textwidth}}
    ...
    \midrule
    1 & Get Base Data & \texttt{tp-get-base-data.yaml} \\
    \addlinespace
    2 & Data Security Check & \texttt{tp-security-check.yaml} \\
    \addlinespace
    3 & Data Preprocessing & \texttt{tp-preprocess-data.yaml} \\
    \addlinespace
    4 & \textbf{Model Training} & \\
      & \hspace{3mm} Train XGBoost & \texttt{tp-train-xgb.yaml} \\
      & \hspace{3mm} Train Balanced RF & \texttt{tp-train-brf.yaml} \\
    \addlinespace
    5 & Compare Models & \texttt{tp-compare-models.yaml} \\
    \bottomrule
\end{tabular*}
\end{table}
...
Quy trình Retraining Pipeline được chúng tôi đề xuất được mô tả trong Hình \ref{fig:chap3-retraining-pipeline.png}. So với Training Pipeline, Retraining Pipeline bổ sung thêm các bước thu thập và hợp nhất dữ liệu production mới, cho phép mô hình cập nhật kiến thức dựa trên dữ liệu thực tế phát sinh từ môi trường vận hành. Tương tự Training Pipeline, các bước của \ac{dag} được ánh xạ tới những template tương ứng, image được lấy từ \ac{dag}, và artifacts được lưu trữ vào S3 artifact bucket theo workflow ID để đảm bảo khả năng truy vết.
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} >{\centering\arraybackslash}p{0.1\textwidth} >{\raggedright\arraybackslash}p{0.35\textwidth} >{\raggedright\arraybackslash}p{0.45\textwidth}}
    ...
    1 & \textbf{Prepare Data} & \\
      & \hspace{3mm} Fetch Production Data & \texttt{tp-fetch-prod-data.yaml} \\
      & \hspace{3mm} Get Base Data & \texttt{tp-get-base-data.yaml} \\
    \addlinespace
    2 & Merge Data & \texttt{tp-merge-data.yaml} \\
    \addlinespace
    3 & Data Security Check & \texttt{tp-security-check.yaml} \\
    \addlinespace
    4 & Data Preprocessing & \texttt{tp-preprocess-data.yaml} \\
    \addlinespace
    5 & \textbf{Model Retraining} & \\
      & \hspace{3mm} Retrain XGBoost & \texttt{tp-train-xgb.yaml} \\
      & \hspace{3mm} Retrain Balanced RF & \texttt{tp-train-brf.yaml} \\
    \addlinespace
    6 & Compare Models & \texttt{tp-compare-models.yaml} \\
    \bottomrule
\end{tabular*}
\end{table}
...
Quy trình Serving Pipeline được chúng tôi đề xuất được mô tả trong Hình \ref{fig:chap3-serving-pipeline.png}. Pipeline này chỉ có một bước là Deploy Model, sử dụng template \textbf{tp-deploy-model.yaml}. Nó đóng vai trò quan trọng trong việc triển khai mô hình đã được chọn từ MLflow Model Registry lên môi trường production.
...
\begin{enumerate}
    ...
    \item \textbf{Deploy to Kubernetes}: Đọc cấu hình manifest cho deployment (k8s/model-serving.yaml), thay thế các placeholders MODEL\_VERSION, DEPLOYMENT\_NAME, CONTAINER\_IMAGE bằng giá trị thực tế, sau đó thực thi lệnh \textbf{kubectl apply} để tạo Deployment với số replicas chỉ định và Service trong namespace mlflow. Mỗi pod sẽ sử dụng image \textbf{mlflow\_serving} và chạy lệnh \textbf{mlflow models serve} để load model từ MLflow Model Registry.
...
Để minh họa hoạt động của hệ thống MLSecOps trong môi trường thực tế, chúng tôi xây dựng ứng dụng web mô phỏng quy trình đăng ký tài khoản và phát hiện gian lận. Luồng xử lý của ứng dụng được mô tả trong Hình~\ref{fig:chap3-app-flow.png}, bao gồm các bước sau:
...
\begin{enumerate}
    ...
    \item Backend của ứng dụng sẽ có một \textbf{Data Handler} được viết bằng Python. Nó sẽ nhận data từ người dùng, sẽ đó tiền xử lý dữ liệu và gọi Model Endpoint
...
Hình \ref{fig:prometheus-home} thể hiện giao diện chính của Prometheus sau khi triển khai. Có thể thấy, giao diện này không quá thân thiện với người dùng trong việc theo dõi các chỉ số hệ thống theo thời gian thực. Đó cũng là lý do cần có Grafana trực quan hóa dữ liệu.

\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.3]{img/chapter3/prometheus-home.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.3]{img/chapter3/grafana-login.png}
