...
\begin{figure}[!htbp]
    ...
    \caption{Ma trận tương quan của tập dữ liệu}
...
\begin{itemize}
    ...
    \item cluster\_autoscaler.tf: Triển khai metrics-server và cluster-autoscaler bằng Helm, mục tiêu nhằm tự động điều chỉnh số lượng node trong cụm EKS dựa trên tải hệ thống.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.34]{img/chapter3/chap3-ci-create-release.png}
...
\begin{itemize}
    ...
    \item \textbf{tp-security-check.yaml}: Sử dụng image security\_check chứa các pre-trained models và logic phát hiện label-flip. Container tải dữ liệu đã gộp từ S3, chạy security\_check.py để phát hiện các mẫu nghi ngờ, và tải danh sách các mẫu nghi ngờ trở lại artifact bucket.
...
\begin{enumerate}
    ...
    \item \textbf{Deploy to Kubernetes}: Đọc cấu hình manifest (k8s/model-serving.yaml) cho deployment, thay thế các placeholders MODEL\_VERSION, DEPLOYMENT\_NAME, và CONTAINER\_IMAGE bằng giá trị thực tế, sau đó thực thi lệnh \textbf{kubectl apply} để tạo Deployment với số replicas chỉ định và Service trong namespace mlflow. Mỗi pod sẽ sử dụng image \textbf{mlflow\_serving} và chạy lệnh \textbf{mlflow models serve} để load model từ MLflow Model Registry.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.34]{img/chapter3/chap3-app-flow.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.27]{img/chapter3/prometheus-home.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.27]{img/chapter3/grafana-login.png}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[scale=0.27]{img/chapter3/grafana-home.png}
