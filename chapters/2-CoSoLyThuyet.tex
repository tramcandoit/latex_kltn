...
Con người đóng vai trò đánh giá và xử lý các ngoại lệ trong quá trình phân tích, phát hiện gian lận. Trong các tình huống mà công nghệ không thể đánh giá một cách chắc chắn, hệ thống sẽ tạo ra các ngoại lệ mà cần con người can thiệp để ra quyết định. Con người cũng chính là yếu tố tạo nên công nghệ.
...
Trong thực tế, các mô hình cơ sở thường được sử dụng trong ensemble learning là các mô hình đơn giản, dễ huấn luyện và có khả năng biểu diễn phi tuyến. Trong đó, \textbf{decision tree} (cây quyết định) là một trong những mô hình được sử dụng phổ biến nhất nhờ cấu trúc linh hoạt, khả năng diễn giải tốt và chi phí tính toán thấp.

Các phương pháp ensemble learning phổ biến có thể được chia thành ba nhóm chính: Bagging, Boosting và Stacking.

\paragraph{b. XGBoost}

Extreme Gradient Boosting (XGBoost) là một thuật toán ensemble learning mạnh mẽ dựa trên phương pháp boosting, trong đó các mô hình cơ sở là các \textbf{decision tree}. XGBoost xây dựng một chuỗi các decision tree theo cách tuần tự, sao cho mỗi mô hình mới được huấn luyện nhằm khắc phục các lỗi của các mô hình trước đó.
...
Random Forest là một thuật toán ensemble learning dựa trên phương pháp bagging, trong đó nhiều \textbf{decision tree} được huấn luyện độc lập với nhau. Mỗi decision tree được xây dựng từ một tập dữ liệu huấn luyện được lấy mẫu ngẫu nhiên có hoàn lại từ tập dữ liệu ban đầu.
...
Balanced Random Forest là một biến thể của Random Forest được đề xuất nhằm cải thiện hiệu suất trong các bài toán phân loại với dữ liệu mất cân bằng. Phương pháp này vẫn sử dụng \textbf{decision tree} làm mô hình cơ sở, nhưng có sự điều chỉnh trong quá trình lấy mẫu dữ liệu huấn luyện cho từng decision tree.
...
\begin{figure}
    \centering
    \includegraphics[scale=0.8]{img/chapter2/container-architecture.png}
    \caption{Kiến trúc của ứng dụng container \cite{what_is_container} }
    \label{fig:chap2-container-architecture}
\end{figure}

Hình \ref{fig:chap2-container-architecture} thể hiện kiến trúc của nhiều ứng dụng container. Trong đó, mỗi ứng dụng A, B, C,... cùng được đóng gói dưới dạng một container riêng biệt và cùng chạy trên một hạ tầng với cùng một hệ điều hành. Mỗi container không chứa toàn bộ hệ điều hành, mà chỉ cùng chia sẻ nhân (kernel). Do đó, container được xem là nhẹ hơn nhiều so với máy ảo. Một container runtime đóng vai trò trung gian giao tiếp giúp các container chạy được trên hệ điều hành của máy chủ. Đây là thành phần có vai trò tạo, thực thi, cung cấp API để quản lý container. Một vài container runtime phổ biến như: Docker, CRI-O, containerd,...
...
\begin{figure}
    \centering
    \includegraphics[scale=0.3]{img/chapter2/docker-flow.png}
    \caption{Mối quan hệ giữa Dockerfile, Docker image và Docker container}
    \label{fig:chap2-docker-flow}
\end{figure}

Hình \ref{fig:chap2-docker-flow} mô tả mối quan hệ giữa Dockerfile, Docker image và Docker container. Theo đó, Dockerfile xây dựng nên Docker image, Docker image sau đó được chạy để tạo thành Docker container, cũng chính là môi trường thực thi của ứng dụng.
...
\begin{itemize}
    \item Bảo vệ dữ liệu huấn luyện khỏi các hành vi thao túng hoặc xâm nhập trái phép
...
Trong bối cảnh các mô hình học máy ngày càng được triển khai rộng rãi, MLSecOps góp phần đảm bảo rằng hệ thống không chỉ hoạt động hiệu quả mà còn an toàn trước các mối đe dọa an ninh.

\paragraph{c. Thách thức}

Các hệ thống học máy hiện nay phải đối mặt với nhiều rủi ro bảo mật, trong đó có các hình thức tấn công như tấn công đầu độc dữ liệu (data poisoning attack)~[3], tấn công adversarial examples
hay khai thác mô hình (model extraction). Những mối đe dọa này có thể làm suy giảm nghiêm trọng hiệu suất và độ tin cậy của mô hình.
